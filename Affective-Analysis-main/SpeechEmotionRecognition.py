...
class speechEmotionRecognition:
    ...
    def __init__(self, subdir_model=None):
        ...
        self.voice_recording(filename = "audio.wav", duration=8)
        predict,timestamp = self.predict_emotion_from_file("audio.wav")
        # print(predict.shape())
        # print(timestamp.shape())
        self.prediction_to_csv(predictions=predict,timestamp = timestamp, filename= "data.csv")
        for emotion in predict:
            print(str(emotion))
        # for t in timestamp:
        #     print(str(t))
    ...
    '''
    def prediction_to_csv(self, predictions, timestamp, filename, mode='w'):

        # Write emotion in filename
        with open(filename, mode) as f:
            if mode == 'w':
                f.write("EMOTIONS, TIMESTAMP"+'\n')
            for i in range(len(predictions)):
                f.write(str(predictions[i])+", "+str(timestamp[i])+'\n')
            f.close()
